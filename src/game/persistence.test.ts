...
describe('game persistence', () => {
  ...
  it('migrates settlements missing meta defaults', () => {
    ...
    expect(loadedKingdom?.policy.peaceDividendUntilTurn).toBe(-1)
    expect(loadedKingdom?.policy.peaceDividendPartnerKingdomId).toBe('none')
    expect(loadedKingdom?.policy.peaceDividendIntensity).toBe(0)
